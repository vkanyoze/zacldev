...
<main class="py-16 flex justify-center">
    <div class="w-8/12">
    <div class="mt-4 text-2xl font-bold text-custom-gray">Change password</div>
    <div class="text-lg mt-2 text-custom-gray">Keep your information up-to-date and ensure secure communication.</div>
    <div>
    <form action="{{ route('password.change') }}" method="POST">
    @csrf
    <label class="block text-custom-gray font-bold mt-4 max-w-2xl" for="full_name"> {{ session('success') }} <label>
    <input type="password" placeholder="New password" name="new_password" class="mt-2 w-full p-3 rounded border border-custom-green focus:outline-none focus:border-blue-600 font-normal">
                            @if ($errors->has('email'))
                            <span class="text-xs tracking-wide text-red-600 font-normal">{{ $errors->first('email') }}</span>
                            @endif
    </div>
    <div>
    <label class="block text-custom-gray font-bold mt-3  max-w-2xl" for="password">Old Password<label>
    <div class="relative w-full mt-1">
                    ...
                    </div> <input id="old_password" name="old_password" type="password" autocomplete="off" placeholder="password" class="js-password mt-2 w-full p-3 rounded border border-custom-green focus:outline-none focus:border-blue-600 font-normal">
                </div>
    ...
    <div class="flex mt-8">
         <button class="px-6 py-4 bg-green-500 text-white rounded font-bold mt-3">Change password</button>
    </div>
    </div>
</form>
<script>
