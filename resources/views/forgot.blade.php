...
<div class="flex items-center justify-center min-h-screen bg-custom-gray">
    <div class="px-10 py-8 text-left bg-white shadow-lg">
        ...
        <h3 class="mt-9 text-2xl font-bold text-center text-custom-gray">Reset password</h3>
        <p class="mt-2 text-center text-custom-gray">Securely reset your password and regain access to your account.</p>
        <form action="{{ route('password.request') }}" method="POST">
            @csrf
            <div class="mt-6">
                <div>
                    <label class="block text-custom-gray font-bold" for="email">Email Address<label>
                            <input type="text" value="{{ old('email') }}" placeholder="your email address linked to account" name="email" class="mt-2 w-full p-3 rounded border {{ $errors->has('email') ? 'border-red-600' : 'border-custom-green' }} focus:outline-none focus:border-blue-600 font-normal">
                ...
                <div class="mt-4 w-full justify-center text-sm text-center text-custom-gray font-normal">
                    You did not see the anything in your email ? <a href="#" class="text-blue-900 hover:underline font-bold">Resend</a>, if you don't have <br>an account <a href="{{ route('login') }}" class="text-blue-900 hover:underline font-bold">Log in</a>
                </div>
            </div>
        </form>
    </div>
